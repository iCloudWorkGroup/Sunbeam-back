...
public class MSheetServiceImpl implements MSheetService {
	...
	public void paste(OuterPaste outerpaste, String excelId, Integer step) {
		...
		if(sortRList.size()<oprRow+rowLen+1){
			...
			for(int i = 0;i<oprRow+rowLen+1-sortRList.size();i++){
				...
				rowCol.setLength(19);
				if(i == 0){
					rowCol.setPreAlias(sortRList.get(sortRList.size()-1).getAlias());
				}else{
		...
		if(sortCList.size()<oprCol+colLen+1){
			...
			for(int i = 0;i<oprCol+colLen+1-sortCList.size();i++){
				...
				rowCol.setLength(69);
				if(i == 0){
					rowCol.setPreAlias(sortCList.get(sortCList.size()-1).getAlias());
				}else{
		...
		//拆分目标区的合并单元格
		List<MRowColCell> relation = mcellDao.getMRowColCellList(excelId, sheetId, rowList, colList);
		...
		//清除非合并单元格的显示内容
		mcellDao.updateContent("displayTexts", null, unspanCellIdList, excelId, sheetId);
	...
	public boolean isAblePaste(OuterPaste outerPaste, String excelId) {
		...
		boolean canPaste = isPaste(excelId,sheetId,oprRow,oprCol,rowLen,colLen);
		return canPaste;
   }
	...
	public boolean isCutCopy(Copy copy, String excelId) {
		...
		String sheetId = excelId+0;
		boolean canPaste = isPaste(excelId,sheetId,targetStartRowIndex,targetStartColIndex,
				rowRange,colRange);
		return canPaste;
	}
	
	private boolean isPaste(String excelId,String sheetId,int targetStartRowIndex,
		int	targetStartColIndex,int rowRange,int colRange){
		...
		if(sortRList.size()<targetStartRowIndex+rowRange+1){
			...
			for(int i = targetStartRowIndex;i < targetStartRowIndex+rowRange+1;i++){
				rowList.add(sortRList.get(i).getAlias());
			}
		}
		//起始位置，超出列
		...
		if(sortCList.size()<targetStartColIndex+colRange+1){
			...
			for(int i= targetStartColIndex;i< targetStartColIndex+colRange+1;i++){
				colList.add(sortCList.get(i).getAlias());
			}
		}
		...
		for(MCell mc:mcellList){
			...
			if(mc.getColspan()>1||mc.getRowspan()>1){
				...
				int rowIndex = rowMap.get(ids[0]);
				int colIndex = colMap.get(ids[1]);
				if(rowIndex+mc.getRowspan()>targetStartRowIndex+rowRange+1){
				 canPaste = false;
				 break;
				}
				if(colIndex+mc.getColspan()>targetStartColIndex+colRange+1){
					 canPaste = false;
					 break;
				}
			}
			
			if((null!=mc.getContent().getLocked())&&(mc.getContent().getLocked())){
				canPaste = false;
				break;
			}
	 ...
	 * @param sheetId
	 * @param step
	 * @param type  0 复制      1 剪切复制
	 */
	private void copyOrCut(Copy copy,String excelId,String sheetId,Integer step,int type){
		int startRowIndex = copy.getOrignal().getStartRow();
		int endRowIndex = copy.getOrignal().getEndRow();
		int startColIndex = copy.getOrignal().getStartCol();
		int endColIndex = copy.getOrignal().getEndCol();
		...
		if(sortRList.size()<targetStartRowIndex+rowRange+1){
			...
			//增加新的行
			for(int i = 0;i<targetStartRowIndex+rowRange+1-sortRList.size();i++){
				maxRow = maxRow+1;
				String row = maxRow+"";
				MRow mrow = new MRow(row,sheetId);
				RowCol rowCol = new RowCol();
		...
		if(sortCList.size()<targetStartColIndex+colRange+1){
			...
			//增加新的列
			for(int i = 0;i<targetStartColIndex+colRange+1-sortCList.size();i++){
				maxCol = maxCol+1;
				String col = maxCol+"";
				MCol mcol = new MCol(col,sheetId);
				RowCol rowCol = new RowCol();
		...
		msheet.setMaxrow(maxRow);
		baseDao.update(excelId, msheet);//更新最大行，最大列
		relationList = mcellDao.getMRowColCellList(excelId, sheetId, rowList, colList);
		for(MRowColCell mrcc:relationList){
			cellIdList.add(mrcc.getCellId());
		}
		//删除目标区域的MCell对象
